// Package cli assembles the coinbase command tree from the coinbase
...
// NewApp assembles the kit application from the coinbase domain. The
...
//
// To add a command, declare it in coinbase/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := coinbase.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(coinbase.Domain{}).Register(app)
