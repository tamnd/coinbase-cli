...
describes what users get. Pick a format with `-o`, or let coinbase choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
coinbase <command> -o table   # aligned columns for reading
coinbase <command> -o jsonl   # one JSON object per line, for piping
coinbase <command> -o json    # a single JSON array
coinbase <command> -o csv     # spreadsheet friendly
coinbase <command> -o tsv     # tab-separated
coinbase <command> -o url     # just the URL column
coinbase <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
coinbase <command> --fields id,title,url
```
...
```bash
coinbase <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
coinbase <command>            # a table, because this is a terminal
coinbase <command> | wc -l    # JSONL, because this is a pipe
```
